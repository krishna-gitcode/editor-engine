...
async function generateAIOutline(topic: string, signal: AbortSignal, onChunk: (text: string) => void): Promise<void> {
  const apiKey = import.meta.env.VITE_OPENROUTER_API_KEY || localStorage.getItem('ee_openrouter_key') || localStorage.getItem('editor_openrouter_api_key') || '';
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: 'deepseek/deepseek-chat', stream: true, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] }),
  });
