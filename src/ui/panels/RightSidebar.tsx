      ...
      {selectedObject ? (
        <div className="space-y-5 text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {['rect', 'textbox'].includes(selectedObject.type || '') && (
              <div className="flex flex-col">
                <span className="text-[11px] text-[var(--ee-text-secondary)]">Corner Radius</span>
                <input
                  type="number"
          ...
          {/* Color & Styling */}
          <div>
            <div className="font-semibold text-[var(--ee-text-primary)] mb-2.5">Fill & Border Styling</div>
            <div className="grid grid-cols-2 gap-2">
        ...
        <div className="space-y-5 text-xs">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1">Border Color</label>
                <input
                  type="color"
                  ...
                  className="w-full h-8 p-1 bg-[var(--ee-surface-2)] border border-[var(--ee-border)] rounded cursor-pointer"
                />
              </div>
              <div>
                <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1">Border Width</label>
                <select
                  value={tableAttrs.borderWidth || '1px'}
                  onChange={(e) => activeEditor.chain().focus().updateTableAttributes({ borderWidth: e.target.value }).run()}
                  className="w-full h-8 bg-[var(--ee-surface-2)] border border-[var(--ee-border)] rounded px-2 text-[var(--ee-text-primary)]"
                >
          ...
          <div className="space-y-3 pt-2 border-t border-[var(--ee-border)]">
            <div className="font-semibold text-[var(--ee-text-primary)] text-[11px]">Table Dimensions & Alignment</div>

            <div>
              <label className="block text-[var(--ee-text-secondary)] text-[10px] mb-1">Table Overall Width</label>
              <div className="grid grid-cols-4 gap-1 mb-1.5">
                {['100%', '75%', '50%', 'auto'].map((w) => (
                  <button
                    key={w}
                    onClick={() => activeEditor.chain().focus().updateTableAttributes({ width: w }).run()}
                    className={`py-1 rounded text-center text-[10px] ${tableAttrs.width === w ? 'bg-indigo-600 text-white font-medium' : 'bg-[var(--ee-surface-2)] hover:bg-[var(--ee-surface-3)] text-[var(--ee-text-primary)]'}`}
                  >
                    {w}
                  </button>
                ))}
              ...
              <input
                ...
                className="w-full bg-[var(--ee-surface-2)] border border-[var(--ee-border)] rounded p-1.5 text-xs text-[var(--ee-text-primary)]"
              />
            </div>

            <div>
              <label className="block text-[var(--ee-text-secondary)] text-[10px] mb-1">Table Position / Alignment</label>
              <select
            ...
            <div>
              <div className="flex justify-between text-[var(--ee-text-secondary)] text-[10px] mb-1">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full accent-emerald-500 h-1.5 bg-[var(--ee-surface-2)] rounded-lg cursor-pointer"
                />
                <button
                  onClick={() => activeEditor.chain().focus().updateTableAttributes({ rotation: 0 }).run()}
                  className="px-1.5 py-0.5 bg-[var(--ee-surface-2)] hover:bg-[var(--ee-surface-3)] text-[10px] text-red-300 rounded"
                >
                  Reset
                </button>
              </div>
            ...
            <div>
              <label className="block text-[var(--ee-text-secondary)] text-[10px] mb-1">Cell Padding</label>
              <select
          ...
          <div className="space-y-2.5 pt-2 border-t border-[var(--ee-border)]">
            <div className="font-semibold text-[var(--ee-text-primary)] text-[11px] flex items-center justify-between">
              <span>Column & Row Dimensions</span>
              <span className="text-[10px] text-emerald-400 font-normal">Active Cell</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] text-[var(--ee-text-secondary)] mb-1">Cell Width (px/% / auto)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] text-[var(--ee-text-secondary)] mb-1">Row Height (px / auto)</label>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-[10px] text-[var(--ee-text-secondary)] mb-1">Cell Background Color</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  onChange={(e) => activeEditor.chain().focus().updateActiveCellAttributes({ backgroundColor: e.target.value }).run()}
                  className="w-12 h-7 bg-[var(--ee-surface-2)] border border-[var(--ee-border)] rounded cursor-pointer"
                />
                <button
                  onClick={() => activeEditor.chain().focus().updateActiveCellAttributes({ backgroundColor: null }).run()}
                  className="px-2 py-1 bg-[var(--ee-surface-2)] hover:bg-[var(--ee-surface-3)] text-[10px] text-[var(--ee-text-primary)] rounded"
                >
                  Clear Color
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <button
                ...
                className="py-1.5 px-2 bg-[var(--ee-surface-2)] hover:bg-[var(--ee-surface-3)] rounded text-center text-[var(--ee-text-primary)] text-[11px]"
              >
                + Col Before
              </button>
              <button
          ...
          <div className="space-y-2 pt-2 border-t border-[var(--ee-border)]">
            <div className="font-semibold text-[var(--ee-text-primary)] text-[11px]">Table Structure</div>
            <div className="grid grid-cols-2 gap-2">
              <button
              ...
              <button
                ...
                className="py-1.5 px-2 bg-[var(--ee-surface-2)] hover:bg-[var(--ee-surface-3)] rounded text-center text-amber-300 text-[11px]"
              >
                Merge Cells
              </button>
              <button
        ...
        <div className="space-y-5 text-xs">
          <div className="border-b border-[var(--ee-border)] pb-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-[var(--ee-text-secondary)] mt-0.5">Dynamically resize & position embedded iframe.</p>
            </div>
            <button
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1 font-semibold">Iframe Source URL</label>
              <input
                type="text"
                value={iframeAttrs.src || ''}
                onChange={(e) => activeEditor.chain().focus().updateAttributes('iframe', { src: e.target.value }).run()}
                className="w-full bg-[var(--ee-surface-2)] border border-[var(--ee-border)] rounded p-2 text-xs text-[var(--ee-text-primary)]"
              />
            </div>

            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1">Width (px)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1">Height (px)</label>
                <input
                  type="number"
            ...
            <div>
              ...
              <input
                ...
                className="w-full accent-cyan-500 h-1.5 bg-[var(--ee-surface-2)] rounded-lg cursor-pointer"
              />
            </div>
            <div>
              <div className="flex justify-between text-[11px] text-[var(--ee-text-secondary)] mb-1">
            ...
            <div className="pt-2 border-t border-[var(--ee-border)]">
              <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1 font-semibold">Iframe Alignment & Wrapping</label>
              <select
            ...
            <div>
              <label className="block text-[var(--ee-text-secondary)] text-[11px] mb-1 font-semibold">Quick Aspect Ratio Presets</label>
